...
const Recurrence = {
	getAll: async () => {
		return db.select().from("Recurrences").leftJoin("Intentions", "Recurrences.id", "Intentions.recurrence_id")
	},
	...
	getById: async (id) => {
		return db.select().from("Recurrences").leftJoin("Intentions", "Recurrences.id", "Intentions.recurrence_id").where("Recurrences.id", id).first()
	},
